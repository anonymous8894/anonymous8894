...
pub fn union_entity_inner(
    ...
    let input_eq_name = Ident::new(&format!("{}Eq", input_name.to_string()), Span::call_site());
        ...
        variant_types.iter().zip(variant_names_snake_case.iter())
    {
        impl_provide_ref_arena.extend(
            quote! {
                impl <#input_life_time> ::fixing_rs_base::symbolic::entities::ProvideRefArena<#variant_type> for #arena_name<#life_time_args> #input_where_clauses {
    ...
    let or_names = if variant_names.len() > 0 {
        quote! {|#((#input_name::#variant_names(_), #input_name::#variant_names(_)))|*}
    } else {
        quote! {}
    };
    ...
    Ok(quote! {
        ...
        #input_vis enum #input_name<#symbolic_lifetime, #input_life_time> #input_where_clauses {
            Empty(::fixing_rs_base::utils::Pointer<#symbolic_lifetime, ::fixing_rs_base::symbolic::entities::EmptyEntity>),
            #(#variant_names(::fixing_rs_base::utils::Pointer<#symbolic_lifetime, #variant_types>),)*
        }

        #input_vis struct #input_eq_name<'a, 'b, #symbolic_lifetime, #input_life_time>(
            &'a #input_name<#symbolic_lifetime, #input_life_time>,
            &'b #input_name<#symbolic_lifetime, #input_life_time>,
        );

        impl<'a, 'b, #symbolic_lifetime, #input_life_time> ::fixing_rs_base::symbolic::rule::Rule for #input_eq_name<'a, 'b, #symbolic_lifetime, #input_life_time> {
            fn send_rule_to<'c>(&self, receiver: &mut (impl ::fixing_rs_base::symbolic::rule::RuleReceiver<'c> + ?Sized)) {
                match (self.0, self.1) {
                    (#input_name::Empty(e1), #input_name::Empty(e2)) => {},
                    #((#input_name::#variant_names(e1), #input_name::#variant_names(e2)) => e1.assert_equals_to(e2).send_rule_to(receiver),)*
                    _ => unreachable!(),
                }
            }
        }
        ...
        impl <#symbolic_lifetime, #input_life_time> Default for #input_name<#symbolic_lifetime, #life_time_args> #input_where_clauses {
            fn default() -> Self {
                Self::Empty(::fixing_rs_base::utils::Pointer::from_ptr(&::fixing_rs_base::symbolic::entities::EmptyEntity))
            }
        }
        ...
        #(
            impl <#symbolic_lifetime, #input_life_time>
                ::fixing_rs_base::symbolic::entities::ContainingSingleEntity<#symbolic_lifetime, #variant_types>
        ...
        impl <#symbolic_lifetime, #input_life_time>
            ::fixing_rs_base::symbolic::entities::ContainingSingleEntity<#symbolic_lifetime, ::fixing_rs_base::symbolic::entities::EmptyEntity>
            for #input_name<#symbolic_lifetime, #life_time_args> #input_where_clauses {
            fn unwrap_single_entity(self) -> ::fixing_rs_base::utils::Pointer<'s, ::fixing_rs_base::symbolic::entities::EmptyEntity> {
                match self {
                    Self::Empty(e) => e,
                    _ => panic!("Expected variant {} but got {}", stringify!(Empty), self.variant_name()),
                }
            }
            fn from_single_entity(entity: ::fixing_rs_base::utils::Pointer<'s, ::fixing_rs_base::symbolic::entities::EmptyEntity>) -> Self {
                Self::Empty(entity)
            }
        }
        ...
        impl <#input_life_time> ::fixing_rs_base::symbolic::entities::EntityUnionContainer for #arena_name<#life_time_args> #input_where_clauses {
        }

        impl<#symbolic_lifetime, #input_life_time>
            ::fixing_rs_base::symbolic::entities::EntityUnion<#symbolic_lifetime>
            for #input_name<#symbolic_lifetime, #life_time_args>
            #input_where_clauses {
            type Container = #arena_name<#life_time_args>;

            fn construct_same(&self, world: &mut ::fixing_rs_base::symbolic::world::SymbolicWorld, container: &#symbolic_lifetime Self::Container) -> Self {
                use ::fixing_rs_base::symbolic::entities::Entity;
                match self {
                    Self::Empty(e) => *self,
            ...
            fn assert_equals_to<'a>(&'a self, other: &'a Self) -> impl ::fixing_rs_base::symbolic::rule::Rule + 'a {
                use ::fixing_rs_base::symbolic::entities::Entity;
                match (self, other) {
                    (Self::Empty(_), Self::Empty(_)) #or_names
                    => #input_eq_name(self, other),
                    _ => {
