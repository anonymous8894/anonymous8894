...
pub fn impl_semantic_processor_inner(
    ...
    let usize_type = syn::parse_str::<Type>("usize").unwrap();
    let graph_edge_type_type = syn::parse_str::<Type>("GraphEdgeType").unwrap();
    ...
    for (symbol_id, procs) in m_symbolic_terminal.into_iter() {
        ...
        symbolic_terminal_quotes.extend(quote! {
            #symbol_id => { self. #func_name ( gprop, inh.into_single_prop(stringify!(#func_name)), literal, edge_weight, edge_type, index).assembly_prop_result() },
        });
    }
