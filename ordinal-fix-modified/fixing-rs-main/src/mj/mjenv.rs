...
use fixing_rs_base::{
    containers::{Map, Set},
    grammar::OwnedToken,
    utils::{Pointer, RefArena, StringPool, StringRef},
};
...
pub struct MJEnv<'a> {
    arena: &'a MJArena<'a>,
    names: Set<StringRef<'a>>,
    classes: Map<StringRef<'a>, MJClsRef<'a>>,
    default_classes: MJDefaultClasses<'a>,
    str_pool: &'a StringPool<'a>,
}
...
impl<'a> MJEnv<'a> {
    pub fn new(
        ...
        ast: Vec<MJAstCls<'_>>,
        str_pool: &'a StringPool<'a>,
    ) -> Result<Self, MJParseError> {
        let mut names = Set::new();
        let default_classes = Self::allocate_default(arena, str_pool, &mut names);
        let mut result = Self {
            arena,
            classes: Map::new(),
            default_classes,
    ...
    fn allocate_default(
        ...
        let object = Self::allocate_cls(arena, OBJECT_CLASS_NAME, true, str_pool, names);
        let void = Self::allocate_cls(arena, VOID_CLASS_NAME, false, str_pool, names);
        let null = Self::allocate_cls(arena, NULL_CLASS_NAME, false, str_pool, names);
    ...
    fn add_default_object(&mut self) {
        let name = self.str_pool.get_or_add(OBJECT_CLASS_NAME);
        self.classes.insert(name, self.default_classes.object);
    }

    fn add_classes(&mut self, ast: &Vec<MJAstCls<'_>>) -> Result<(), MJParseError> {
        for ast_cls in ast.iter() {
            let name = self.str_pool.get_or_add(ast_cls.name);
    ...
    fn set_class(&mut self, ast: &Vec<MJAstCls<'_>>) -> Result<(), MJParseError> {
        for ast_cls in ast.iter() {
            let ast_cls_name = self.str_pool.get_or_add(ast_cls.name);
            let cls = *self.classes.get(&ast_cls_name).unwrap();
            let inh_cls_name = self.str_pool.get_or_add(ast_cls.inh);
            let super_cls = self.classes.get(&inh_cls_name);
            match super_cls {
                Some(super_cls) => {
            ...
            for ast_field in ast_cls.fields.iter() {
                let ast_field_ty_name = self.str_pool.get_or_add(ast_field.ty);
                let ty = self.classes.get(&ast_field_ty_name).copied();
                ...
                let name = self.str_pool.get_or_add(ast_field.name);
                self.names.insert(name);
                match ty {
                    Some(ty) => {
...

            {
                ...
                let params: Result<Vec<_>, _> = ast_cls
                    ...
                    .iter()
                    .map(|x| {
                        let cls_name = self.str_pool.get_or_add(x);
                        let ty = self.classes.get(&cls_name).copied();
                        match ty {
                            Some(ty) => Ok(ty),
            ...
            for ast_method in ast_cls.methods.iter() {
                ...
                let ret_ty = match ast_method.ret_ty {
                    Some(ret_ty) => {
                        let cls_name = self.str_pool.get_or_add(ret_ty);
                        let ty = self.classes.get(&cls_name).copied();
                        match ty {
                            Some(ty) => Some(ty),
                            None => {
                ...
                let name = self.str_pool.get_or_add(ast_method.name);
                self.names.insert(name);
                let method = MJMethod {
                    cls,
                    name,
    ...
    pub fn get_class(&self, name: StringRef<'a>) -> Option<MJClsRef<'a>> {
