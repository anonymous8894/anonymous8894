    ...
    G: EdgeWeightGenerator<'a, 'b>,
{
    ...
    fn nti_expr_0_0(&self, _g: &PropArray<CProp>, inh: &CSymTab<'b>) -> CIdSelector<'b> {
    ...
    fn nti_expr_7_0(&self, _g: &PropArray<CProp>, inh: &CSymTab<'b>) -> CIdSelector<'b> {
    ...
    // nts args : 0
    fn nts_args_0(&self, _g: &PropArray<CProp>, inh: &CArgs<'b>) -> Option<PropEmpty> {
        match inh.can_zero() {
            true => Some(PropEmpty),
            false => None,
        }
    ...
    fn nti_args_1_0(&self, _g: &PropArray<CProp>, inh: &CArgs<'b>) -> Option<CArgs<'b>> {
        inh.to_zero_more()
    }

    // nti 0 argsOther : 0 expr
    fn nti_argsOther_0_0(&self, _g: &PropArray<CProp>, inh: &CArgs<'b>) -> Option<CSymTab<'b>> {
    ...
    fn nti_argsOther_1_0(&self, _g: &PropArray<CProp>, inh: &CArgs<'b>) -> Option<CSymTab<'b>> {
    ...
    fn nti_declOp_1_0(&self, _g: &PropArray<CProp>, inh: &CDeclInh<'b>) -> CDeclInh<'b> {
    ...
    fn nti_declId_0_0(&self, _g: &PropArray<CProp>, inh: &CDeclInh<'b>) -> Option<CIdSelector<'b>> {
    ...
    fn nts_declId_3(
        ...
        _s1: &PropEmpty,
        _s2: &PropEmpty,
        decl_id: &CDeclIdSyn<'b>,
    ) -> CDeclIdSyn<'b> {
        decl_id.clone()
    }

    // nti 0 declId : 4 declId [ ]
    fn nti_declId_4_0(&self, _g: &PropArray<CProp>, inh: &CDeclInh<'b>) -> Option<CDeclInh<'b>> {
    ...
    fn nti_declId_2_0(&self, _g: &PropArray<CProp>, inh: &CDeclInh<'b>) -> Option<CDeclInh<'b>> {
    ...
    // nts for1 : 0 ;
    fn nts_for1_0(&self, _g: &PropArray<CProp>, inh: &CSymTab<'b>, _s1: &PropEmpty) -> CSymTab<'b> {
        inh.clone()
    }

    // nti 0 for1 : 1 decl
    fn nti_for1_1_0(&self, _g: &PropArray<CProp>, inh: &CSymTab<'b>) -> CSymTab<'b> {
        inh.new_scope()
    }
    ...
    fn nti_functionBody_0_0(&self, _g: &PropArray<CProp>, _inh: &PropEmpty) -> CSymTab<'b> {
        CSymTab::from_map(
            self.env
                .globals()
                .iter()
        ...
        match inh {
            CIdSelector::FuncName(sym_tab) => match literal {
                ...
                None => self
                    ...
                    .values()
                    .flat_map(|func| {
                        let func_valid = (current_identifiers.is_none()
                            || current_identifiers.as_ref().unwrap().contains(&func.name))
                ...
                None => sym_tab
                    ...
                    .flat_map(|(name, _)| {
                        if (current_identifiers.is_none()
                            || current_identifiers.as_ref().unwrap().contains(name))
                            && self.env.is_true_id(*name)
                        {
            ...
            CIdSelector::NewIdentifier(sym_tab) => {
                if let Some(literal) = literal {
                    ...
                    self.env
                        ...
                        .flat_map(|id| {
                            if current_identifiers.is_none()
                                || current_identifiers.as_ref().unwrap().contains(id)
                            {
                                match sym_tab.get(*id) {
                                    Some(s) => {
                                        if s.current_scope() {
                                            None
                                        } else {
