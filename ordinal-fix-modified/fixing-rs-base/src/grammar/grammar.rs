...
impl<'a> Grammar<'a> {
    ...
    fn add_rule(
        ...
        match rule_type.length() {
            ...
            GrammarRuleLength::One => {
                ...
                right1.add_ref_one(rule);
            }
            GrammarRuleLength::Two => {
                let right1 = right1.unwrap();
                let right2 = right2.unwrap();
                right1.add_ref_two_left(rule);
                right2.add_ref_two_right(rule);
            }
        }
        rule
    }
    ...
    fn add_multi_value_symbol(&mut self, symbol: SymbolRef<'a>) {
        if !symbol.is_multi_valued() {
            self.multi_value_symbols.push(symbol);
            symbol.set_multi_valued();
        } else {
    ...
    fn add_element<'input>(&mut self, element: &'input Element<'input>) -> SymbolRef<'a> {
        self.add_symbol(element.element_type, &*element.get_element_value())
    }
    ...
    fn gen_element_literal_values(ast: &GrammarFile<'_>) -> Result<(), ParseError> {
        for rule in ast.rules.iter() {
            for alt in rule.alternatives.iter() {
                for sym in alt.elements.iter() {
                    sym.gen_element_literal_value()
                        .map_err(|e| ParseError::EscapeError(sym.element_value.to_string(), e))?;
                }
            }
        }
        Ok(())
    ...
    fn build_grammar_from_ast<'input>(&mut self, ast: &GrammarFile<'_>) -> Result<(), ParseError> {
        Self::gen_element_literal_values(ast)?;
    ...
    fn construct_epsilon_symbols(&self) -> HashSet<SymbolRef<'a>> {
        ...
        while !epsilon_symbols_queue.is_empty() {
            ...
            for rule in symbol.ref_one().iter() {
                if !epsilon_symbols.contains(&rule.left()) {
                    epsilon_symbols_queue.push_back(rule.left());
                }
            }
            for rule in symbol.ref_two_left().iter() {
                if !epsilon_symbols.contains(&rule.left())
            ...
            for rule in symbol.ref_two_right().iter() {
                if !epsilon_symbols.contains(&rule.left())
