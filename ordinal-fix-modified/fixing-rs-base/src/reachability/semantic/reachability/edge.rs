    ...
    PSS: UnionProp,
{
    ...
    fn find_or_add_edge<'t>(
        ...
        let fedge_length = if allow_multiple { gedge.length() } else { 0 };
        ...
        if !edges.contains_key(&edge_key) {
            ...
            let edge_entity = FEntity::new(edge_key, gedge, gedge.length(), literal);
    ...
    pub(in super::super) fn assembly_result(
        ...
        if !allow_multiple && edge.length() > gedge.length() {
