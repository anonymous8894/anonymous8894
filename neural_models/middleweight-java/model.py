...
class ModifyingModel(torch.nn.Module):
    def __init__(self):
        ...
        self._bert_config = BertConfig(
            vocab_size=mjtokens.num_tokens, pad_token_id=mjtokens.v_pad
        )
        self.bert = BertModel(self._bert_config, add_pooling_layer=False)
        self.ff = torch.nn.Linear(768, 3072)
        self.output = torch.nn.Linear(3072, mjtokens.num_tokens)
        self.insertions = torch.nn.Linear(
            3072, mjtokens.num_tokens * hyper_parameters.num_insertions
        )

    def forward(self, x):
        batch_size, sequence_length = x.shape
        attention_mask = (x != mjtokens.v_pad).long()
...
def modifying_model_loss(outputs, insertions, target_outputs, target_insertions):
    padding = target_outputs == mjtokens.v_pad
...
class ModifyingModelDataset:
    def __init__(self, file_name, length_limit=None, shuffle=False, shuffle_seed=None):
        with open(file_name, "rb") as f:
            self._data = pickle.load(f)
        self._length = len(self._data)
        if length_limit is not None:
            self._length = min(self._length, length_limit)
...
class ModifyingModelTrainer:
    def __init__(
        ...
        if load_dataset:
            dataset_training = ModifyingModelDataset("train.pkl")
            dataset_validation = ModifyingModelDataset("valid.pkl")
            dataset_test = ModifyingModelDataset(
                "test.pkl",
                length_limit=test_length_limit,
    ...
    def get_model_file_name(self):
        return os.path.join(os.path.dirname(__file__), "model.pt")
    ...
    def train_epoch(self):
        ...
        for inputs, outputs, insertions in t:
            ...
            if self.writer is not None:
                self.writer.add_scalar("Loss/train", loss.item(), self.number_steps)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            if valid_loss < self.lowest_valid_loss:
                ...
                self.save_model()
            if self.writer is not None:
                self.writer.add_scalar("Loss/valid", valid_loss, self.number_steps)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for inputs, outputs, insertions in tqdm.tqdm(
                ...
                insertion_empty += (
                    (insertions_p == mjtokens.v_remove)
                    .masked_fill(padding_mask.unsqueeze(-1), False)
                    .sum()
                    .item()
                )
            ...
            if self.writer is not None:
                self.writer.add_scalar(
                    "Output_Acc", output_corr / output_total, self.number_steps
                )
                self.writer.add_scalar(
                    "Insertion_Acc", insertion_corr / insertion_total, self.number_steps
                )
