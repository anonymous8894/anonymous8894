...
class ModifyingModelDataset:
    ...
    def __getitem__(self, idx):
        data = self._data[idx]
        inputs, outputs, insertions = (data.inputs, data.outputs, data.insertions)
...
class ModifyingModelTrainer:
    def __init__(
        ...
        if load_dataset:
            dataset_training = ModifyingModelDataset("checkout/c-train.pkl")
            dataset_validation = ModifyingModelDataset("checkout/c-valid.pkl")
    ...
    def get_model_file_name(self):
        return os.path.join(os.path.dirname(__file__), "c_model.pt")
    ...
    def train_epoch(self):
        ...
        for inputs, outputs, insertions in t:
            ...
            if self.writer is not None:
                self.writer.add_scalar("C/Loss/train", loss.item(), self.number_steps)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            if valid_loss < self.lowest_valid_loss:
                ...
                self.save_model()
            if self.writer is not None:
                self.writer.add_scalar("C/Loss/valid", valid_loss, self.number_steps)
        ...
        with torch.no_grad():
            ...
            if self.writer is not None:
                self.writer.add_scalar(
                    "C/Output_Acc", output_corr / output_total, self.number_steps
                )
                self.writer.add_scalar(
                    "C/Insertion_Acc",
